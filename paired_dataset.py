...
class AlignedVolumesDataset(torch.utils.data.Dataset):
    def __init__(self, *volumes, protocals, \
        ...
        self.mask_under = np.array(sio.loadmat(u_mask_path)['mask'])
        self.s_mask_up = np.array(sio.loadmat(s_mask_up_path)['mask'])
        self.s_mask_down = np.array(sio.loadmat(s_mask_down_path)['mask'])
